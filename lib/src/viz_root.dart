...
class VizRoot {
  ...
  static Future<VizRoot> forDirectory(String path,
      {bool flagOutdated: false, List<String> ignorePackages: const []}) async {
  ...
  String toDot(
      {bool escapeLabels: false, List<String> ignorePackages: const []}) {
    _update();
...
Iterable<Dependency> _allDeps(VizRoot root, List<String> ignorePackages) sync* {
  for (var pkg
